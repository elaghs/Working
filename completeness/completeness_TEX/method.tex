...
Now we focus on the relationship between \nondetcov\ and proof-based metrics.  Considering the conjunctive structure of transition relation, assigning a ``fresh'' input in \nondetcov\ coverage is an isomorphic operation to simply removing a $T_i$ from $T$, which makes 
a mutant transition relation like the one introduced in Definition \ref{def:non-det}. 
...
\end{lemma}
\begin{proof}
\mike{obvious?} \ela{   not so sure if obvious with my formalization?}
\end{proof}
