...
Considering the definition of a \mivc , a brute-force technique for enumerating all \mivc s would involve exploring the power set of $T$ (denoted by $ \mathcal{P}(T) $) to determine the provability of a
given property by any subset of $T$, which would be computationally infeasable.
However, this section proposes a complete
enumeration algorithm that only needs to explore a small portion of $\mathcal{P}(T)$
in order to compute $AIVC$.  The algorithm follows the structure of the MARCO approach proposed for generating all minimal unsatisfiable subsets (MUSes) in~\cite{marco2016fast}, but adapts the proofs to our setting.  To this end, we begin by introducing several additional notions and definitions, most of which are analogous or equivalent to those in~\cite{marco2016fast}.
...
Next, we tie the {\em map} results back to the minimal IVCs problem. Note that, from Theorem~\ref{thm:minimal-hard}, finding a minimal IVC is undecidable if the original checking problem is undecidable.  In most cases, if we are able to prove a property against a full model, we are able to conclusively determine its MIVCs, but it is not guaranteed.  For a given set $S$, if our implementation is unable to prove the property, we conservatively assume that the property is falsifiable.  We discuss how our implementation manages undecidability in more detail in Section~\ref{sec:impl}.

\begin{lemma}
\label{lem:map:comp}
For decidable model checking problems $(I, T) \vdash P$, $map$ is satisfiable iff
...
\begin{corollary}
\label{cor:map:cc}
For decidable model checking problems $(I, T) \vdash P$, $map$ is unsatisfiable iff every $S \in
...
\label{theorem:termination}
  For decidable model checking problems, Algorithm~\ref{alg:aivc} will terminate.
\end{theorem}
\begin{proof}
...
\begin{theorem}
\label{theorem:aivc}
  For decidable model checking problems, Algorithm \ref{alg:aivc} enumerates all \mis es and \mivc s.
\end{theorem}
\begin{proof}
