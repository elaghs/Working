...
For the sake of simplicity, hereafter, we refer a \emph{minimal} set of inductive validity core as an "IVC" set.
Note that there could be many IVC sets for a given property, corresponding to different proofs. To capture that notion, we define \emph{all IVC sets ($AIVC$)} for a property as an association to all its IVCs.
...
We use the set membership notation to define different coverage functions. We define coverage notions in terms of a function $\zeta$ that maps a property $P$ to $S \subseteq T$. Given $\zeta(P)$, $\forall P \in \Delta,\quad T_i \in T$, we say $T_i \in \zeta(P)$ \emph{iff} $P$ covers $T_i$ according to $\zeta$,
 otherwise $T_i \notin \zeta(P)$.
...
%formalized in Definition \ref{def:single-mut} as $\zeta_{nc}$.
