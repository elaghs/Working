...
\ela{in continue:\\describing algorithm\\say what maximal sat model is\\say in the maxsat algo, M is a literals that are true in maximal model\\ \\Maximal Models The first two goals were addressed in both [21] and [28] similar fashions: generate seeds by computing large or maximal models of map . A maximal model of map is one in which no False assignment can be made True without violating a clause; therefore, a maximal model will represent a subset of C for which all supersets have been explored\\ in implementation talk about max sat, must list, initialization of map \\In exploring the power set,we first define several functions; function \isadeq ~tells whether or not property $P$ is provable by a subset of $T$:$$\isadeq : \mathcal{P}(T) \rightarrow \{true~(adequate), false~(inadequate)\}$$ \noindent Function \checksat ~determines if an existentially quantified formula is satisfiable or not \footnote{We assume readers are familiar with the boolean satisfiability problem, which is the problem of determining whether there exists an assignment that satisfies a given propositional formula. For more information, refer to \cite{cook1971complexity}.}: $$\checksat : Boolean \rightarrow \{ true~(\sat), false~(\unsat) \}$$\\say why the algorithm is fast; because we block down and up based on MIS and IVC, which prunes the maximum number of cases from the search space (map)}
