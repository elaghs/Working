...
A netlist can immediately be mapped a transition system consisting of a conjunction of equations, where each vertex $v_i \in VN$ is represented by a variable $V_i$ in the transition system, and each non-input vertex is assigned by an equation based on its type and input edges.  Given this representation, we can define nondeterministic coverage as follows:

\begin{definition} {\emph{Nondeterministic coverage (\nondetcov) ~\cite{chockler2010coverage}.} }
\label{def:non-det}
Given a set of netlist equations $T_i = (V_i = E_i)$ and ``fresh'' model variable \inputnode, 
$T_i \in T$ is covered by property $P$ \emph{iff} $T_i \in \nondetcov (P)$, where
$\nondetcov (P) = \{ T_i~|~ T_i = (V_i = E_i)\in T \wedge (I, T \setminus \{(V_i = E_i)\} \cup \{(V_i = \textrm{\inputnode})\}) \nvdash P\}$.
\end{definition}
...
\noindent It is straightforward to define stuck-at-true and stuck-at-false coverage by replacing \inputnode\ by either constant 1 or 0.
