...
\begin{itemize}
    ...
    \item \textbf{RQ2:} How hard are different metrics to satisfy ($\zeta_{ivc}$ vs $\zeta_{sm}$)?  And, what is the relationship between the coverage scores obtained from different notions and provability?
\end{itemize}

\subsection{Experimental Setup}

In order to compare our new coverage notion ($\zeta_{ivc}$) with the previous one ($\zeta_{sm}$), we designed an experiment from a suite of 700 Lustre models developed
