...
The high-level goal of this research is to add to the usability of the symbolic model checkers. We are specifically concerned with the scenarios where a model checker establishes the correctness proof of a given property.
 When it comes to verification, if the answer to a correctness query is positive, in most tools,
further information is not provided. What we would like to provide is
...
negative result. Such an explanation should be both formal and human-understandable. We would like to equip the tools with a mechanism to show why a proved property is valid. From one point of view, reasoning about the proofs is not a new idea: UNSAT cores~\cite{zhang2003extracting}
...
The anticipated contributions of the work are therefore as follows:
\begin{itemize}
...
\section{Evaluation}
We intend to implement all of the inductive validity core algorithms in an industrial model checker called JKind ~\cite{jkind},
