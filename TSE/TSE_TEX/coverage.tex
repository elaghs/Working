...
\mike{Does this belong back in the preliminaries section??}
\ela{I think it would be better if we don't merge these with the background. If we put these things in the background, the reader may just feel there are too many irrelevant/useless formalizations in the beginning... but still if you'd like, I could merge them}
...
To conclude this section, we should mention that one can define many more proof-based coverage metrics based on the $\mivc /AIVC$ idea. Metrics that make use of the $AIVC$ relation are computationally more expensive to compute than \ivccov\ although they result in higher coverage scores.
