...
%
To implement the All-IVCs procedure \aivcalg, we have extended \texttt{JKind} with a new engine that implements Algorithm \ref{alg:aivc} using \texttt{Z3}.
...
be handled in any implementation of the \bfalg, \ucbfalg, and \aivcalg~algorithms involves the undecidability of the model checking problem;
...
The timeout we set for each iteration of the \bfalg\ and \aivcalg\ algorithms is ($30$ sec  $+\ 5\ \times$ (\emph{proof-time} $+$ \emph{\ucalg-time})).
