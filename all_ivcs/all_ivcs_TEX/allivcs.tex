...
respectively, from consideration. When a clause is added to $map$, thec
orresonding $S \in \mathcal{P}(T)$ is called \emph{explored}.
The supersets or subsets of $S$ which are blocked from
consideration are called \emph{excluded}. The remaining elements
of $\mathcal{P}(T)$ are \empty{unexplored}.
...
\begin{algorithm}[t]
 ...
 % $L \leftarrow \varnothing$ \\
  ...
  \While{$\checksat (map)$} { \label{alg:aivc:checksat}
    ...
    $S \leftarrow \bigcup_{a_i \in M} \actlit ^{-1}(a_i)$ \\ %\mike{should we use a letter other than M?}\\
\BlankLine
    \If{$\isadeq (P, S)$}{
    \BlankLine
      $S' \leftarrow \getivc (P, S)$ \\
