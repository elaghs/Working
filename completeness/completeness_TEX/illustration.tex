...
Our first (somewhat faulty) attempt at formalization is as follows:
...
We are still missing the \abovehyst, \aoneabove, \atwoabove, and \dtwo\ variables, so the `above' thresholds are irrelevant to our properties.  Examining \offp, we realize that we have a specification error; the DOI should be off if either \inhibit\ is true or both altimeters are above the threshold, so we fix it:
