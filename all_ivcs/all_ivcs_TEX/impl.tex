...
we treat $S$ as an \emph{inadequate} set to ensure correctness of results: we do not know 
whether such a set will lead to a proof or an eventual counterexample.
When we mark an indeterminate set $S$ as inadequate, line 13 will prune off $S$ and all 
its subsets from the search space.  Since the timeout is used by both the brute-force 
algorithm and the \aivcalg\ algorithm, minimal IVC results are only guaranteed if there 
...
experiment.  It is important to note that by increasing the timeout, it is possible that 
