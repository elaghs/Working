...
\begin{description}
    ...
    \item[Robustness Analysis:] As proposed by Murugesan et. al in~\cite{Murugesan16:renext}, it is possible partition the model elements into MUST and MAY sets based on whether they are in every IVC or only some IVCs, respectively.  This allows insight into the structure of the model vs. the property.  For example, if the MUST set is empty, then the requirement has been implemented in multiple ways, such as would be expected in a fault-tolerant system.  The relative scrutiny of aspects in the model could be driven by their centrality to proofs.
