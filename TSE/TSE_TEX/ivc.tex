...
Note that, given $(I, T) \vdash P$, $P$ always has at least one \mivc, and it may also have many distinct {\mivc}s corresponding to different proof paths. To capture the latter, the \emph{all {\mivc}s ($\mathit{AIVC}$)} relation has been introduced in \cite{Murugesan16:renext}.
\begin{definition}{\emph{All {\mivc}s ($\mathit{AIVC}$):}}
    \label{def:allivcs}
    Given $(I, T) \vdash P$, $AIVC(P)$ is the set of all \mivc s for $P$:
    $$ \mathit{AIVC}(P) \equiv  \{\ S~|~S \subseteq T \land  MIVC(P, S)\} $$
\end{definition}
