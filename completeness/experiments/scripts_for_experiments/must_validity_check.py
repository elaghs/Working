# This is to check which models fail on their Must set

__author__ = "Elaheh"
__date__ = "$Aug 19, 2016 6:27:09 PM$"
...
RESULTS_DIR = 'must_check_results' 
...
def run_single_jkind(file_path):
    args = ['java', '-jar', jkind_jar, '-jsupport', 
            '-solver', 'z3', 
            '-use_unsat_core', file_path + '_mustIvc.xml',
            file_path]
    with open("debug.txt", "a") as debug:
        ...
        try:
            shutil.move (os.path.join(file_path + "_notValidMustc.xml"), RESULTS_DIR) 
        except: 
            pass
