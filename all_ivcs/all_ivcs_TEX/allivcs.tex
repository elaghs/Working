...
Obviously, for every inadequate set $S \subset T$, there is some set $C \subseteq T$ such that $S \cup C$ is adequate; we refer to such set $C$ as a \emph{correction set} for $S$. Furthermore, considering the definition of \ivc, every set $S \in AIVC (P)$ is a \emph{minimal} adequate set meaning  that $\nexists S' \subset S.~ (I, S') \vdash P$.
...
Let $map$ is satisfiable with a model $M$. Then, set
 ...
 so it is or contains an unexplored \ivc.
Case 2: Suppose $S$ is inadequate. Again by Lemma 2, $S$ is either a \mis\ or a subset of a \mis\ that is not yet explored.

On the other hand, let $S \subseteq T$ be an unexplored set. Case 1: Suppose $S$ is an \ivc.
...
\begin{algorithm}[t]
 ...
 % $L \leftarrow \varnothing$ \\
  ...
  \While{$\checksat (map)$} { \label{alg:aivc:checksat}
    ...
    $M \leftarrow \bigcup_{a_i \in V} \actlit ^{-1}(a_i)$ \\ %\mike{should we use a letter other than M?}\\
\BlankLine
    \If{$\isadeq (P, M)$}{
    \BlankLine
    ...
    \Else{
      $map \leftarrow map \wedge (\bigvee_{T_{i}\in (T \setminus M)} \actlit (T_i))$ \label{alg:aivc:iadd} \\
...
Suppose that $x$ is an undiscovered \ivc\ in $map$ after some number of iterations (including 0) of the algorithm.   By Lemma~\ref{lem:map:comp}, $map$ is satisfiable, so the loop proceeds.  In each iteration of the \texttt{while} loop, Algorithm \ref{alg:aivc} extracts a model $M$ from $map$ in line~\ref{alg:aivc:maxsat}, which has not been previously explored, by Lemma~\ref{lem:map:sound}.  If $M$ is inadequate, then it yields a \mis\ (by virtue of using MaxSAT).  This \mis\ is blocked in line~\ref{alg:aivc:iadd}.  By Lemma~\ref{lem:map:comp}, $map$ is still satisfiable, the loop proceeds with a smaller number of remaining sets to add to $map$, and by induction, $x$ is eventually discovered.  Suppose alternately that $M$ is adequate.  If $M = x$, we are finished.  Alternately, $M$ is blocked in line~\ref{alg:aivc:aadd}, $map$ is still satisfiable, and by induction $x$ is eventually discovered.
...
the minimal adequate sets (\ivc s), all we need is to remove any supersets of other sets in $A$.  We can do this ``on the fly'' by changing 
