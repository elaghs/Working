...
LEGENDS =  ['Minimization after ALL_IVCs',
            'Minimization + ALL_IVCs', 'Minimization in IVC_UCBF', '#of IVC sets discovered by ALL_IVCs']
...
for indx, val in enumerate(ucbf):
    ucbf_dic.append({'id': indx, 'val': (val- ucpr_time[indx])})
...
plt.subplots_adjust(hspace=0.1)
ax1 = plt.subplot(111)
#plt.plot(x_axis, sorted_all_ivcs, '-.+g', markersize=5, label=LEGENDS[1]) 
'''plt.plot(x_axis,  sorted_mini, ':rx', markersize=10, label=LEGENDS[0])  
plt.plot(x_axis, sorted_ucbf, '-b' , markersize=11, label=LEGENDS[2]) 
plt.plot(x_axis, sorted_num, ':ko' , markersize=4, label=LEGENDS[3])'''
########################################################################################
plt.plot(x_axis, np.sort(sorted_mini), ':rx', markersize=10, label=LEGENDS[0])
plt.plot(x_axis, sorted_ucbf, '-b' , markersize=11, label=LEGENDS[2]) 
