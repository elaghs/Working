...
\subsubsection{RQ3}
For this research question, we analyzed the minimality of the produced IVC by each algorithm.  Results are shown in Figure~\ref{fig:size}.  Since \mike{XXX} of the models had only one IVC, for these models, the size of the minimum model produced by the \aivcalg\ algorithm should be the same as the \ucbfalg\ algorithm.  For the remainder, even when multiple IVCs were produced, in only 11 cases did the \aivcalg\ produce smaller minimal IVCs.  For these 11 models, the smallest model was \mike{XX\%} the size of the model produced by \ucbfalg, and in the most dramatic case, the number of elements shrank from \mike{XX} to \mike{YY}.
