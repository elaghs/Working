...
A net-list can immediately be mapped into a transition system with a $T$ with a top-level conjunction structure, where each $e_i \in E_N$ is represented by a top-level conjunct $T_i$ in the transition relation.
...
$E_N$ with a new set of edges $E_M$ in the mutant net-list.
Accordingly, the transition system of a mutant design can be constructed w.r.t $E_M$. That is to say, the mutant transition relation would have one additional top-level conjunction
...
\label{def:non-det}
Given transition system $(I, T)$ and a mutant transition relation $T_M$ of $T$,
$T_i \in T$ is covered by property $P$ \emph{iff} $T_i \in \nondetcov (P)$, where
$\nondetcov (P) = \{ T_i~|~ T_i \in T \wedge (I, T) \vdash P \wedge (I, T_M) \nvdash P \}$.

%Given a set of net-list equations $T_i = (V_i = E_i)$ and ``fresh'' model variable \inputnode,
%$T_i \in T$ is covered by property $P$ \emph{iff} $T_i \in \nondetcov (P)$, where
