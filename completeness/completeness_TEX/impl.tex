...
first proves the property and obtains all the invariants generated for the proof (line 1). Since not all these invariants are essential for the proof, it performs some minimization to keep a minimal set of necessary invariants (line 2). The proofs of these invariants themselves are dependent on the structure of the model or design elements that we called inductive validity core (Definition \ref{def:ivc}).
So, using function $map\_to\_design$, Algorithm \ref{alg:uc}
computes design elements those design elements. And finally, it minimizes these computed elements so to obtain a minimal IVC set (line 4). Although Algorithm \ref{alg:uc} performs minimization, because of the structure of the inductive proofs, \ucalg might still not produce an \emph{exact} minimal IVC set \footnote{for more information, see \cite{Ghass16}. Here we abstract away the details.}.
...
based on Definition \ref{def:coverage-justi}.
And, the output of \mustalg represents covered design elements according to Definition \ref{def:non-det}, which is the same as $MUST(P)$.
