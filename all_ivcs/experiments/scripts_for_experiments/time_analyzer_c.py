...
writer = open(os.path.join(ANALYSES_DIR, 'timing_analyses.txt'), 'w')
...
#
writer = open(os.path.join(ANALYSES_DIR, 'overhead.txt'), 'w') 
...
LEGENDS =  ['JKind verification (no IVC computation)', 
            ...
            'JKind verification + IVC_UCBF','JKind verification + All_IVCs']
'''  
...
'''
######################################################################################## 
plt.plot(x_axis, sorted(must_w_proof) , 'gx' , markersize=8, label=LEGENDS[2]) 
plt.plot(x_axis, sorted(aivc), '--k' , markersize=200, label=LEGENDS[4])  
plt.plot(x_axis, sorted(ucbf), ':b.' , markersize=6, label=LEGENDS[3])  
plt.plot(x_axis,  sorted(ucpr_time), 'm+' , markersize=7, label=LEGENDS[1])  
plt.plot(x_axis, sorted(proof_time),'k', markersize=5 , label=LEGENDS[0])
########################################################################################
