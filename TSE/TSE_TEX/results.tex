...
In the previous section, we presented several algorithms for determining
single or all inductive validity cores.  For single \mivcs, we started from a baseline brute-force algorithm that is guaranteed minimal, but is often very slow.  We then constructed an UNSAT-core-based algorithm \ucalg, and a hybrid algorithm \ucbfalg.  
