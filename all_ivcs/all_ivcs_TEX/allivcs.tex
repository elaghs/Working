...
\begin{algorithm}[t]
 ...
 % $L \leftarrow \varnothing$ \\
  ...
  \While{$\checksat (map)$} { \label{alg:aivc:checksat}
    ...
    $M \leftarrow \bigcup_{a_i \in V} \actlit ^{-1}(a_i)$ \label{alg:aivc:assignm} \\ %\mike{should we use a letter other than M?}\\
\BlankLine
    \If{$\isadeq (P, M)$}{
    \BlankLine
      $S' \leftarrow \getivc (P, M)$ \label{alg:aivc:getivc} \\
...
Suppose that $x$ is an undiscovered \ivc\ in $map$ after some number of iterations (including 0) of the algorithm.
