...
if not os.path.exists(MINING_DIR):
    print(MINING_DIR + " does not exist! first, try to run mining scripts.")
    sys.exit(-1)
...
for model in models:
    ...
    runs = reader ['ucbf_minimization_info']
    v = []
    i = []
    u = []
    all = []
...
# Analyze results
#
writer = open(os.path.join(ANALYSES_DIR, 'ucbf_minimization_info.txt'), 'a')
...
plt.yscale('log')
plt.title('running time of different runs in IVC_UCBF')
plt.tight_layout()
plt.grid(True)
