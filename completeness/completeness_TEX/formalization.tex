...
when property $P$ satisfied by $N$ fails on the mutant net-list $(V_N, E^{v_i}_N, \tau^{v_i}_{N})$ constructed by a single non-deterministic mutation from $\tau^{v_i}_{N}$, it is said that a mutant is discovered for $P$ (or $v_i$ is covered by $P$).
